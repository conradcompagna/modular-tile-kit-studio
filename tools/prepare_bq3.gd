...
## Prepare the night revision through native terrain, surface records and measured prop occupancy without installing failures.
func run() -> Dictionary:
	var bridge: Node = Engine.get_main_loop().root.find_child("MTSStudioBridge", true, false)
	var plugin: Variant = bridge.plugin
	var raw: Dictionary = plugin.board._read_board_json("res://exports/blackridge_quarantine_revision03/board_candidate.json")
	...
	var output := FileAccess.open("res://exports/blackridge_quarantine_revision03/native_prepare_report.json",FileAccess.WRITE)
	output.store_string(JSON.stringify(result,"  "))
	output.close()
	if not overlaps.is_empty():
		return result
	board.save_json("res://exports/blackridge_quarantine_revision03/board_prepared.json")
	return result
