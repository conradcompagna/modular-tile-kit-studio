...
BQART = ROOT / 'assets/blackridge_quarantine_revision03'
MATERIAL_ART = ROOT / 'assets/blackridge_quarantine_revision'
OUT = ROOT / 'exports/blackridge_quarantine_revision03'
ASSETS = {}
WINDOW_AUDIT = []
CURRENT_HOUSE = 'prison'
BASELINE=json.loads((OUT/'board_before_revision03.json').read_text())
SUPPORT_AUDIT=[]
random.seed(9411)
STONE = material('BQ3 weathered urban umber brick',(.24,.20,.17),MATERIAL_ART/'exterior_brick_chord/albedo.png')
CAP = material('BQ3 rain-polished pale city stone',(.29,.28,.25),MATERIAL_ART/'lime_plaster_chord/albedo.png')
PLASTER = material('BQ3 soot-stained lime plaster',(.40,.37,.30),MATERIAL_ART/'lime_plaster_chord/albedo.png')
ROOF = material('BQ3 rain-worn blue slate',(.15,.18,.21),ROOT/'assets/blackridge_quarantine/slate_roof_chord/albedo.png')
BANNER = material('BQ3 oxblood institutional canvas',(.17,.024,.026),rough=.94)
WINDOW = material('BQ3 few lit tenement windows',(.42,.20,.065),emission=.45)
GLASSDARK = material('BQ3 unlit barred windows',(.023,.034,.035),rough=.35)
LETTER = material('BQ3 aged ivory lettering',(.49,.43,.30),rough=.94)
...
def finish_literal(name):
    ...
    ASSETS[name]={'asset_id':'BQ3_'+name.upper()+('_GROUNDED' if name.startswith('tenement_row_') else ''),'path':str(BQART/(name+'.glb')).replace('\\','/'),
    ...
    PARTS.clear()
    print('BQ3_ASSET '+json.dumps(ASSETS[name]),flush=True)
...
def window_front(x,z,h,w=1.1,hh=1.5,lit=False,bars=False,boarded=False):
    WINDOW_AUDIT.append({'host':CURRENT_HOUSE,'x':x,'front':z,'bottom':h,'width':w,'height':hh,'lit':lit,'bars':bars,'boarded':boarded,'detail_projection_m':.26})
    ...
    if boarded:
        # Every plank ends over its own jamb and remains below the window head; no multi-storey decorative diagonals.
        for i in range(2):
            beam('Fitted nailed window plank',(x-.025,z+.20,h+.27+i*.55),(x+w+.025,z+.20,h+.53+i*.55),.15,WOOD)
            for xx,yy in ((x+.02,h+.282+i*.55),(x+w-.02,h+.518+i*.55)):
                box('Visible boarding nail',(xx-.018,z+.28,yy-.018),(xx+.018,z+.31,yy+.018),IRON,.003,False)
...
def tenement(w,z_start,base,eave,ridge,variant,west):
    ...
    CURRENT_HOUSE=('west' if west else 'east')+' house '+str(variant)
    start=len(PARTS); depth=5.45+(variant%3)*.17
    # Full-depth masonry rises from flat native support to each stepped street doorway.
    cuts=sorted({0.0,w}|{float(k)-(3+z_start) for k in range(math.ceil(3+z_start),math.floor(3+z_start+w)+1) if 0<float(k)-(3+z_start)<w})
    terrain=BASELINE['terrain']; tw=terrain['size_cells'][0]; ox=terrain['origin_cell'][0]
    for a,b in zip(cuts,cuts[1:]):
        global_z=int(math.floor(3+z_start+(a+b)*.5))
        idx=global_z*tw-ox
        street_height=max(terrain['top_heights'][4*idx:4*idx+4])
        native_height=0.0
        xa,xb=(w-b,w-a) if west else (a,b)
        box('Native-height matched continuous foundation',(xa,0,native_height),(xb,6,base+.12),STONE,0,False)
        SUPPORT_AUDIT.append({'host':CURRENT_HOUSE,'world_z_cell':global_z,'native_height':native_height,'foundation_bottom':native_height,'adjacent_street_height':street_height,'foundation_top':base+.12,'front_reaches_lot_boundary':True})
    ...
    for row in range(storeys):
        ...
        for j in range(count):
            ...
            # A ground-floor doorway owns its complete opening; no window sill crosses its leaf.
            if row==0 and xx+.93+.12>doorx-.08 and xx-.12<doorx+.84+.08:
                continue
    ...
    doorx=w*.45
    box('Solid tenement door',(doorx,depth+.025,base+.13),(doorx+.84,depth+.12,base+1.95),WOOD,.014,False)
    for k in range(5):
    ...
    # The old multi-storey patch timbers crossed glass and frames; fitted window planks now carry this damage language.
...
for x in (0,18):
    # The outward wing faces are inset8cm behind their projecting stone bands, avoiding coincident material faces.
    box('Projected prison wing',(x+.08 if x==0 else x,.12,.42),(x+10 if x==0 else x+9.92,6.42,11.7),STONE,.025,False)
    for band in (.45,3.20,6.65,10.4,11.7):
        box('Prison wing stone stringcourse',(x-.0,.03,band),(x+10,6.55,band+.25),CAP,.024,False)
    # Keep all frames/sills between the engaged buttresses and the reserved wall-banner bay.
    for xx in ([3.10,5.60,8.00] if x==0 else [19.0,21.4,23.8]):
        for h in (4.0,7.4):
...
for x in (10,17.3):
    box('Door jamb foundation',(x,6.2,.46),(x+.70,7.2,6.45),CAP,.028,False)
...
box('Prison name stone plaque',(10.55,6.65,7.46),(17.45,6.83,9.34),CAP,.02,False)
text_front('BLACKRIDGE',14,6.831,8.56,.63)
text_front('HOUSE OF CORRECTION',14,6.831,7.85,.34)
...
propaganda(.98,6.66,10.2,1.70,6.9,['ORDER','PRESERVES'])
propaganda(25.0,6.66,10.2,1.70,6.9,['YOUR SILENCE','PROTECTS US'])
# The central building span needs a real slate roof behind the stone pediment, not the exposed brick block top.
mesh('Central left slate roof',[world(9.73,-.0,12.64),world(9.73,6.90,12.64),world(14,6.90,14.43),world(14,-.0,14.43)],[(0,1,2,3)],ROOF)
mesh('Central right slate roof',[world(14,-.0,14.43),world(14,6.90,14.43),world(18.27,6.90,12.64),world(18.27,-.0,12.64)],[(0,1,2,3)],ROOF)
finish_literal('prison_facade')

(OUT/'kit.json').write_text(json.dumps(ASSETS,indent=2))
(OUT/'window_construction_audit.json').write_text(json.dumps(WINDOW_AUDIT,indent=2))
(OUT/'row_support_source_audit.json').write_text(json.dumps(SUPPORT_AUDIT,indent=2))
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'bq3_architecture.blend'))
print('BQ3_ARCHITECTURE_COMPLETE',flush=True)
