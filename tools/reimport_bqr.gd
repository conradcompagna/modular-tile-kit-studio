...
func run(reimport: bool = true) -> Dictionary:
	var paths: PackedStringArray = PackedStringArray(JSON.parse_string(FileAccess.get_file_as_string("res://exports/blackridge_quarantine_revision/reimport_paths.json")))
	if reimport:
	...
	var file := FileAccess.open("res://exports/blackridge_quarantine_revision/load_checks.json",FileAccess.WRITE)
