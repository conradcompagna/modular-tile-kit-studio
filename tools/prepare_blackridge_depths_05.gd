...
func run() -> Dictionary:
	...
	var document: Dictionary = plugin.board._read_board_json("res://exports/blackridge_depths/board_restored_candidate.json").data
	...
	board.save_json("res://exports/blackridge_depths/board_restored_prepared.json")
	var report: Dictionary = {"ok": true, "props": board.props.size(), "cells": board.terrain.filled_cell_count(), "report": prepared.report}
	var output := FileAccess.open("res://reports/blackridge_depths/native_validation_restored.json",FileAccess.WRITE)
